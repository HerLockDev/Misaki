...
def createApp (connect):
    appname = "misakiuserbot" + str(time() * 1000)[-4:].replace(".", "") + str(random.randint(0,500))
    try:
...
async def oturumacvebotlogolustur (stri, aid, ahash):
    try:
        ...
        Photo = await Client.upload_file(file='misaki.jpg')
...
if __name__ == "__main__":
    ...
    SyperStringKey = "MisakiUBOT"
    GiperStringKey = "HerLockDev/"
    InvalidKey = "http://github.com/" 
    str1 = InvalidKey+GiperStringKey+SyperStringKey

    if os.path.isdir("./MisakiUBOT/"):
        rm_r("./MisakiUBOT/")
    repo = Repo.clone_from(str1,"./MisakiUBOT/", branch="master")
    ...
    config['TZ'] = TZ
    config['TZ_NUMBER'] = "1"
    config['UPSTREAM_REPO_URL'] = "https://github.com/HerLockDev/MisakiUBOT"
    config['SEVGILI'] = "None"
    config['WARN_LIMIT'] = "3"
